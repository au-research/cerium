...
class ScholixRelationChangeExecutorTest {
	...
	void detect_positive() {
        ...
        assertThat(ScholixRelationChangeExecutor.detect(before, after, myceliumService)).isTrue();
	}

    @Test
	void detect_nochange() {
        // no change
        assertThat(ScholixRelationChangeExecutor.detect(new RecordState(), new RecordState(), myceliumService)).isFalse();
	}

    @Test
	void detect_notChanged() {
        Vertex p1 = new Vertex(UUID.randomUUID().toString(), RIFCSGraphProvider.RIFCS_ID_IDENTIFIER_TYPE);
        p1.setObjectClass("party");
        Vertex c1 = new Vertex(UUID.randomUUID().toString(), RIFCSGraphProvider.RIFCS_ID_IDENTIFIER_TYPE);
        c1.setObjectClass("collection");
        ...
        RecordState before = new RecordState();
        before.setOutbounds(Collections.singleton(afterRelation));
        RecordState after = new RecordState();
        after.setOutbounds(Collections.singleton(afterRelation));

        assertThat(ScholixRelationChangeExecutor.detect(before, after, myceliumService)).isFalse();
	}
    ...
    void handle_calls_event() {
        ...
        ScholixRelationChangeSideEffect sideEffect = new ScholixRelationChangeSideEffect("1");
        ScholixRelationChangeExecutor executor = new ScholixRelationChangeExecutor(sideEffect, myceliumService);
        executor.handle();
